...
class EnemySpawner {
    ...
    var baseSpawnInterval: Float = 1.9f
    var timeElapsed: Float = 0f
    var bossSpawned: Boolean = false

    var nextSurgeTimer: Float = 24f
    ...
    fun update(
        ...
        // Faster escalation keeps pressure on upgraded castles without overwhelming the first seconds.
        val currentInterval = (baseSpawnInterval - (timeElapsed / 180f) * 1.25f).coerceAtLeast(0.48f)
        ...
        if (spawnTimer <= 0f && currentEnemyCount < maxEnemies) {
            ...
            val formation = when (currentWave) {
                1 -> if (Random.nextFloat() < 0.45f) SpawnFormation.ARCHER_BATTERY else SpawnFormation.SCATTERED
                2 -> if (Random.nextFloat() < 0.58f) SpawnFormation.SHIELD_WALL else SpawnFormation.ARCHER_BATTERY
                3 -> {
                    val r = Random.nextFloat()
                    if (r < 0.40f) SpawnFormation.SAPPER_RUSH else if (r < 0.74f) SpawnFormation.SHIELD_WALL else SpawnFormation.ARCHER_BATTERY
                }
        ...
        // 2. Periodic reinforcement surge: coordinated flanking squads from opposite sides.
        ...
        // 3. Boss Encounter at the Wave 4 milestone.
        if (timeElapsed >= 95f && !bossSpawned) {
            bossSpawned = true
            val bossAngle = Random.nextFloat() * 6.28318f
            val bossX = castleX + cos(bossAngle) * 700f
            val bossY = castleY + sin(bossAngle) * 700f
    ...
    private fun spawnSquadAtAngle(
        ...
        val distance = Random.nextFloat() * 130f + 590f
